...
var tableCommon = map[string]string{
	"&": "&amp;",
	"<": "&lt;",
	">": "&gt;",
	" ": "&nbsp;",
...
var tableCompat = map[string]string{
	"\"": "&quot;",
	"¨":  "&uml;",
	"´":  "&acute;",
	"·":  "&middot;",
...
func fncGetHtmlTranslationTable(ctx phpv.Context, args []*phpv.ZVal) (*phpv.ZVal, error) {
	...
	if encodingArgs.HasArg() && encodingArgs.Get() != "UTF-8" {
	...
	for k, v := range tableBase {
		result.OffsetSet(ctx, phpv.ZString(k), phpv.ZStr(v))
	}

	// TODO: ENT_NOQUOTES, ENT_QUOTES
	// TODO: sort by character code

	if flags&(ENT_COMPAT) > 0 {
		result.OffsetSet(ctx, phpv.ZString("\""), phpv.ZStr("&quot"))
	}
	if flags&(ENT_COMPAT|ENT_HTML401|ENT_XHTML|ENT_HTML5) > 0 {
		for k, v := range tableCommon {
			result.OffsetSet(ctx, phpv.ZString(k), phpv.ZStr(v))
		}
	}
	if flags&(ENT_COMPAT|ENT_HTML401|ENT_XHTML) > 0 {
		for k, v := range tableCompat {
			result.OffsetSet(ctx, phpv.ZString(k), phpv.ZStr(v))
		}
	}
	if flags&(ENT_HTML5) > 0 {
