...
func prepareRegexp(pattern string) (*gopcre2.Regexp, *pcreError) {
	// Check empty pattern
	if len(pattern) == 0 {
	...
	for _, f := range phpFlags {
		...
		case 'i': // case insensitive
			flags |= gopcre2.Caseless
		case 'm': // multiline
			flags |= gopcre2.Multiline
		case 's': // dotall (. matches newline)
			flags |= gopcre2.DotAll
		case 'x': // extended (ignore whitespace and comments)
			flags |= gopcre2.Extended
		case 'u': // UTF-8 + Unicode properties
	...
	re.SetMatchLimit(10_000_000)
	re.SetDepthLimit(250)
